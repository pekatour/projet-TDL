...
struct
  ...
  let string_of_fonction (Fonction(t,n,lp,li)) = (string_of_type t)^" "^n^
    " ("^((List.fold_right (fun (t,n,v) tq -> (string_of_type t)^" "^n^
     (match v with 
      | None -> ""
  ...
  let string_of_programme (Programme (varGlobales, fonctions, instructions)) =
    (List.fold_right (fun i tq -> (string_of_instruction i)^tq) varGlobales "")^
    (List.fold_right (fun f tq -> (string_of_fonction f)^tq) fonctions "")^
    (List.fold_right (fun i tq -> (string_of_instruction i)^tq) instructions "")
